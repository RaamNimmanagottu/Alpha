...
logger = logging.getLogger("alpha.ema_crossover_13_34_signal")
...
def get_signal(symbol: str, candles: pd.DataFrame) -> Signal:
    """EMA(13)/EMA(34) crossover: BUY when the fast EMA crosses above the slow
    EMA, SELL when it crosses below, WAIT otherwise.

    Backtested as the best of 27+ strategies on 1000 days of real SENSEX 5-min
    data (TP=1200/SL=576, 53.0% win rate, Rs159.07/trade net of realistic
    F&O costs -- see RULES.md).
    """
    ...
    logger.info(
        "%s: ema13=%.2f ema34=%.2f (prev ema13=%.2f ema34=%.2f) cross_up=%s cross_down=%s",
